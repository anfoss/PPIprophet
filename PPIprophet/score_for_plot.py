...
def vec_wd_score(arr, norm, mu=False):
    """
    vectorized wd score
    Args:
        arr: 1d array
    ...
    if mu:
        ...
        return np.sum((arr - mu_) ** 2) + ((mu_ ** 2) * npres)
...
def calc_wd_matrix(m, iteration=1000, q=0.9, norm=False, plot=False):
    ...
    mu = np.array([vec_wd_score(m[i], norm, True) for i in range(m.shape[1])])
    mse = np.array([vec_wd_score(m[i], norm, False) for i in range(m.shape[1])])
    return mu, mse
...
def process_tb(m, ids, outf, nm):
    l = dict(zip(ids, m))
    df = pd.DataFrame.from_dict(l, orient='index').reset_index()
    # print(df)
    # assert False
    ...
    df.to_csv(os.path.join(outf, nm), sep="\t", index=False)
...
def runner(tmp_, outf, crapome, thresh):
    ...
    mu, mse = calc_wd_matrix(m, iteration=10000, q=0.30, norm=False, plot=False)
    process_tb(mu, ids, outf, 'mu_uri.txt')
    process_tb(mse, ids, outf, 'mse_uri.txt')
